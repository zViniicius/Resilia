...
[...$('[data-bs-toggle="tooltip"]')].map(
  trigger => new bootstrap.Tooltip(trigger)
); // ToolTip Info Bell

$(async function () {
  const staffs = Object.values(infoStaffs),
    staffId = [];
  var s;
  for (s = 0; s < staffs.length; s++) {
    ...
    $(`#staffId_${s}`).append(
      `<a data-bs-toggle="modal" data-bs-target="#modal_${s}"><img class="avatar-profile" src=${staffId[s].avatar} width="120"></a>`
    );
    $(`#staffId_${s}`).append(`<h3>${staffId[s].nome}</h3>`);
    criarModais(id, nome, cargo, localidade, linkedIn, github, bio, avatar);
  }
  const alunos = Object.values(infoAlunos),
    user = [];
    ...
    try {
      await fetch(`//api.github.com/users/${user[i].login}`)
        ...
        .then(result => {
          ...
          $('#studentlist').append(
            `<div class=students id=student_${login}></div>`
          );
          $(`#student_${login}`).append(
            `<a data-bs-toggle="modal" data-bs-target="#modal_${login}"><img  class="avatar-profile" src="${avatar_url}" width="120"></a>`
          );
        ...
        .catch(err =>
          console.log(
            `O login ${user[i].login} retornou um erro: ${err.message}`
          )
        );
    } catch {
...
function criarModais(
  ...
  $('#modais')
    .append(`<div class="modal fade" id="modal_${login}" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered">
      <div class="modal-content">
        ...
        <div class="modal-body">
          <img class="avatar-profile" src="${avatar}" width="120">
          <h3>${name}</h3>
